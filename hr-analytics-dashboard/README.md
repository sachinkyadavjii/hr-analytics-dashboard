...
### Secure User Account Creation (No Public Sign-Up)
There is intentionally **no public registration** anywhere in this app.

- **Only ADMIN/HR can create accounts**, from a dedicated, audited User
  Management page — ADMIN can create Admin/HR/Manager/Employee accounts,
  HR can only create Manager/Employee accounts. This is enforced
  **server-side** (`models/user.py: CREATABLE_ROLES`), not just hidden in
  the UI — verified by automated tests that HR cannot create an Admin
  account even by posting the form directly.
- **Accounts link to an existing Employee record** (optional, e.g. for
  utility/admin logins) — only employees without an existing login appear
  in the picker, so duplicate employee records are never created.
- **Temporary password + forced change on first login**: HR/Admin sets an
  initial password when creating an account or resetting one; the user
  must set their own password before accessing anything else. This is
  enforced by a `before_request` hook that blocks every route except
  `/change-password` and `/logout` — not just a UI redirect — so it can't
  be bypassed by typing another URL directly.
- **Account status** (Active / Inactive / Suspended) is separate from
  employment status — deactivating a login never touches HR/attendance/
  payroll history, and accounts are never hard-deleted.
- **Audit log** (ADMIN only) tracks account creation, deactivation/
  reactivation, role changes, and password resets — with actor, target,
  and timestamp. Passwords are never written to the audit log in any form.
- **Forgot Password** is an informational page pointing to HR/Admin, not a
  self-service email reset — since no email system is wired up, a public
  "enter your email to reset" flow would be a real vulnerability (anyone
  could reset anyone else's password).

Verified with an automated security test suite covering: no public
sign-up route exists, Employee/Manager blocked from User Management,
HR blocked from creating Admin accounts, inactive accounts can't log in,
wrong email/password rejected, passwords are hashed (never plaintext),
direct-URL access to restricted pages is blocked, logout clears the
session, and the temporary-password flow can't be bypassed.
